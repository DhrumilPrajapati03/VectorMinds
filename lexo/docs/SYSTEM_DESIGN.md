...
- Render hosts both the frontend and backend as separate web services, plus a managed Postgres instance — keeping the whole stack on one platform as specified.
...
- **Postgres** (Render managed) — all structured data: users, documents, reports, flags, citations, missing clauses, action items, refresh tokens.
...
| Secrets | Render environment groups shared across services (`GEMINI_API_KEY`, `EXA_API_KEY`, `ELEVENLABS_API_KEY`, `WISPR_API_KEY`, `DATABASE_URL`, JWT signing secret, object storage credentials) |
...
| **5 — Deployment & hardening** | Render deployment of both services + managed Postgres, rate limiting, delete/retention flows, disclaimers, monitoring |
