...
flowchart LR
    ...
    WebSpeech[Browser Web Speech API<br/>STT stays in browser]

    User -->|"credentials, files (HTTPS)"| FE
    FE -->|"mic audio → transcript only in browser"| WebSpeech
    FE -->|"JWT + JSON (incl. text questions; REST)"| BE
...
|---|---|---|
| Browser → Frontend | Credentials, uploaded files; mic audio for STT stays in the browser (Web Speech) | Frontend is public/untrusted client code; never place secrets here |
| Frontend → Backend | JWT access token, JSON/multipart bodies; **text questions only** for voice Q&A (no raw STT audio) | Only trusted server component (backend) holds credentials to everything downstream |
...
| Backend → Gemini | Document text/clause content; voice Q&A question text + report context | Contract text leaves Lexo's infrastructure for analysis — see §7 |
...
| Backend → STT provider | *(none)* — no server-side STT; Wispr unused | Mic audio for transcription never leaves the browser |
...
| `POST /api/voice/transcribe` | Yes — **501 unsupported** (client-side Web Speech; not used in demo) |
| `POST /api/voice/ask` | Yes — text question in (no audio) |
...
- **Third parties:** uploaded contracts are treated as sensitive/confidential documents. As part of normal product operation:
  ...
  - **Voice audio for STT stays in the browser** (Web Speech API). The backend receives **text questions only** for voice Q&A — no raw mic audio is uploaded for transcription. Wispr / server-side STT is not used.
...
- **Dependency/secrets hygiene:** secrets (`JWT_SECRET`, `DATABASE_URL`, `GEMINI_API_KEY`, `EXA_API_KEY`, `ELEVENLABS_API_KEY`, S3 credentials) are read from `.env` only, never hardcoded in source, never committed, never echoed into logs or error responses (workspace rule `general.mdc`; `SYSTEM_DESIGN.md` §8). `WISPR_API_KEY` is unused/optional (STT is browser Web Speech) and must not be required to run the app.
...
| **4 — Voice** | Voice Q&A grounding constraint (§9); TTS to ElevenLabs; STT audio stays in browser (§7) |
